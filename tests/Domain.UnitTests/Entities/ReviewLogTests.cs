...
public class ReviewLogTests
{
    ...
    {
        var log = new ReviewLog(Guid.NewGuid(), DateTimeOffset.UtcNow, 4, 10, 2.5, 3);

        log.CardId.Should().Be(Guid.NewGuid());
    ...
    {
        Action act = () => new ReviewLog(Guid.NewGuid(), DateTimeOffset.UtcNow, 6, 5, 2.5, 1);
    ...
    {
        Action act = () => new ReviewLog(Guid.NewGuid(), DateTimeOffset.UtcNow, 3, 5, 1.0, 1);
